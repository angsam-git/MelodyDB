...
#    return redirect(url_for('.album', album = session['album_id']))
  ...
  g.conn.execute('INSERT INTO comment(text, comment_id, user_id, album_id, song_id) VALUES (%s, %s, %s, %s, null)', text, comment_id + 1, session['user_id'], album_id)
  return redirect(url_for('.album', album = session['album_id']))
...
#    return redirect(url_for('.song', song = session['song_id']))
  ...
  for result in cursor:
    comment_id = result['comment_id']
  g.conn.execute('INSERT INTO comment(text, comment_id, user_id, album_id, song_id) VALUES (%s, %s, %s, null, %s)', text, comment_id + 1, session['user_id'], song_id)
  return redirect(url_for('.song', song = session['song_id']))
